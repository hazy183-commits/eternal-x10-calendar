...
const familyFor = name => {
  if (/Dark Crystal/i.test(name)) return 'dark_crystal';
  if (/Tallum/i.test(name)) return 'tallum';
  if (/Nightmare/i.test(name)) return 'nightmare';
  if (/Majestic/i.test(name)) return 'majestic';
  if (/Imperial Crusader/i.test(name)) return 'imperial_crusader';
  if (/Draconic Leather/i.test(name)) return 'draconic_leather';
  if (/Major Arcana/i.test(name)) return 'major_arcana';
  return 'other';
};
...
const slug = value => String(value || '').toLowerCase().normalize('NFKD').replace(/[\u0300-\u036f]/g, '').replace(/['’]/g, '').replace(/[^a-z0-9]+/g, '_').replace(/^_+|_+$/g, '');
