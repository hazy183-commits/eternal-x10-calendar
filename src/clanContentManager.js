...
const STAFF=new Set(['owner','admin','leader']);
...
export function installClanContentManager(supabase){
  ...
  async function boot(zone,side,main){
    ...
    const homeCard=[...main.querySelectorAll('.zone-card')].find(x=>x.textContent.includes('OGŁOSZENIA KLANOWE'));
    if(homeCard){
    ...
    const panel=document.createElement('section');panel.className='zone-view ob-content-panel';panel.dataset.zonePanel='content-editor';panel.innerHTML=`
    ...
    let role='member';let settings={};let announcements=[];

    const getRole=async()=>{const p=await profile();role=String(p?.role||'member').toLowerCase();nav.hidden=!(p?.status==='approved'&&role==='owner');return p};
    const isStaff=()=>STAFF.has(role);
    ...
    const applySettings=()=>{
      const sub=main.querySelector('header p');if(sub&&settings.welcome_subtitle)sub.textContent=settings.welcome_subtitle;
      const quote=main.querySelector('header em');if(quote&&settings.clan_quote)quote.textContent=settings.clan_quote;
    ...
    annPanel?.addEventListener('click',async e=>{const id=Number(e.target.dataset.annEdit||e.target.dataset.annPin||e.target.dataset.annToggle||e.target.dataset.annDelete||0);if(!id||!isStaff())return;const a=announcements.find(x=>x.id===id);if(!a)return;if(e.target.dataset.annEdit){zone.querySelector('[data-zone-view="content-editor"]')?.click();setTimeout(()=>showAnnouncementForm(a),0);return}let r;if(e.target.dataset.annPin)r=await supabase.from('clan_announcements').update({is_pinned:!a.is_pinned,updated_at:new Date().toISOString()}).eq('id',id);if(e.target.dataset.annToggle)r=await supabase.from('clan_announcements').update({is_active:!a.is_active,updated_at:new Date().toISOString()}).eq('id',id);if(e.target.dataset.annDelete&&confirm('Usunąć to ogłoszenie?'))r=await supabase.from('clan_announcements').delete().eq('id',id);if(r&&!r.error)await loadAnnouncements()});
    ...
    supabase.auth.onAuthStateChange(()=>setTimeout(async()=>{await getRole();await loadSettings();await loadAnnouncements()},100));
