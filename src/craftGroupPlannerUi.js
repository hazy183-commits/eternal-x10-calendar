...
export function installCraftGroupPlannerUi(supabase) {
  ...
  const refresh = async (feedback = '', { focusProject = false } = {}) => {
    ...
    try {
      ...
      renderGroup(section, workspace, selectedIndex, feedback);
      if (focusProject) focusProjectSection(section);
    } catch (error) {
      section.innerHTML = `<div class="craft-group-empty"><b>Nie udało się wczytać grupowego craftingu.</b><p>${escapeHtml(error?.message || error)}</p></div>`;
    } finally {
      loading = false;
    }
  };
