...
function previewMarkup(preview) {
  ...
  return `<article class="ob-discord-preview-message">
    ...
    <div class="ob-discord-preview-grid">
      <p><small>📅 DATA</small><b>${esc(preview.start_date || String(preview.start || '').split(' ')[0] || '—')}</b></p>
      <p class="highlight"><small>⏰ START</small><b>${esc(preview.start_time || String(preview.start || '').split(' ')[1] || '—')}</b></p>
