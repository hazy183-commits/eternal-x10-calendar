...
export function installAdminDashboard(supabase) {
  ...
  async function resolveProfile(){
    ...
    userTab.hidden=currentProfile?.role!=='owner'||currentProfile?.status!=='approved';
    return currentProfile;
  }
  async function loadUsers(){
    await resolveProfile();
    const list=document.querySelector('#ownerUsersList'),msg=document.querySelector('#ownerUsersMessage');
    if(currentProfile?.role!=='owner'){list.innerHTML='';msg.textContent='Ta sekcja jest dostępna tylko dla Ownera.';return;}
  ...
  const observer=new MutationObserver(async()=>{if(document.querySelector('#adminModal').classList.contains('open')){await resolveProfile();if(currentTab==='users'&&currentProfile?.role!=='owner')showTab('events');else if(currentTab==='events'){listView.hidden=false;formView.hidden=true;}}});
  observer.observe(document.querySelector('#adminModal'),{attributes:true,attributeFilter:['class']});
