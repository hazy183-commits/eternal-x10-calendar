...
export function installMemberRoster(supabase){
  ...
  panel.innerHTML=`<div class="zone-section-head"><small>DOWÓDZTWO KLANU</small><h3>CZŁONKOWIE KLANU</h3><p>Lista kont, ról i statusów. Owner może zmieniać role, blokować konta i usuwać członków z klanu.</p></div><div class="ob-roster-shell"><div class="ob-roster-toolbar"><div class="ob-roster-stat"><b data-rs="all">0</b><span>Razem</span></div><div class="ob-roster-stat"><b data-rs="member">0</b><span>Członkowie</span></div><div class="ob-roster-stat"><b data-rs="leader">0</b><span>Liderzy</span></div><div class="ob-roster-stat"><b data-rs="admin">0</b><span>Admini</span></div><input id="obRosterSearch" class="ob-roster-search" placeholder="Szukaj po nicku..."></div><p id="obRosterMessage" role="status"></p><div id="obRosterList" class="ob-roster-list"><div class="ob-roster-empty">Ładowanie…</div></div></div>`;
  ...
  panel.addEventListener('click',async e=>{const d=e.target.closest('[data-roster-remove]');if(d){d.disabled=true;const msg=panel.querySelector('#obRosterMessage');try{const target=rows.find(row=>row.id===d.dataset.id);if(target&&await removeMember(supabase,target)){await load();msg.textContent='Członek został usunięty z klanu.';window.dispatchEvent(new Event('orzel:member-removed'));}}catch(error){msg.textContent='Błąd: '+error.message;}finally{d.disabled=false;}return;}const r=e.target.closest('[data-roster-role]');if(r){update(r.dataset.id,{role:r.dataset.rosterRole,status:'approved'});return}const b=e.target.closest('[data-roster-block]');if(b){update(b.dataset.id,{status:'blocked'});return}const u=e.target.closest('[data-roster-unblock]');if(u)update(u.dataset.id,{status:'approved'})});
