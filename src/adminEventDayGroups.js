...
const warsawParts=()=>{
  const parts=new Intl.DateTimeFormat('en-CA',{timeZone:'Europe/Warsaw',year:'numeric',month:'2-digit',day:'2-digit',hour:'2-digit',minute:'2-digit',hour12:false}).formatToParts(new Date());
...
function relativeLabel(dateKey){
  ...
  if(!Number.isFinite(d.getTime()))return 'DZIEŃ';
...
function filterMatches(e){
  ...
  if(f!=='Wszystkie'&&e.type!==f)return false;
  if(q&&!`${e.name||''} ${e.boss||''} ${e.location||''}`.toLowerCase().includes(q))return false;
  return true;
}
function rowHtml(e){
  const time=String(e.event_time||'').slice(0,5);
  return `<div class="admin-event"><div class="admin-event-copy"><div class="event-thumb" data-boss-name="${esc(e.boss||e.name)}"><span>ART</span></div><div><b>${esc(e.name)}</b><span>${esc(e.event_date)} · ${esc(time)} · ${esc(e.type)}</span><span>${esc(e.location||'Brak lokalizacji')}</span></div></div><div><button class="edit-btn" data-edit="${esc(e.id)}">Edytuj</button><button class="delete-btn" data-delete="${esc(e.id)}">Usuń</button></div></div>`;
}
...
export function installAdminEventDayGroups(supabase){
  ...
  async function refresh(){
    ...
    try{
      ...
      const {data,error}=await supabase.from('events').select('id,name,type,boss,event_date,event_time,location').gte('event_date',now.date).order('event_date',{ascending:true}).order('event_time',{ascending:true});
      if(error||!Array.isArray(data))return;
      let rows=data.filter(e=>{
        const t=String(e.event_time||'').slice(0,5);
        return e.event_date>now.date||(e.event_date===now.date&&t>=now.time);
      }).filter(filterMatches);
      const sort=document.querySelector('#adminSort')?.value||'nearest';
      if(sort==='latest')rows=[...rows].reverse();
      else if(sort==='name')rows.sort((a,b)=>String(a.name).localeCompare(String(b.name),'pl'));

      const groups=new Map();
      for(const e of rows){if(!groups.has(e.event_date))groups.set(e.event_date,[]);groups.get(e.event_date).push(e)}
