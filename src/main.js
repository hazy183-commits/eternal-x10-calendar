...
function renderCalendarWeek() {
  ...
  strip.innerHTML = Array.from({ length: 7 }, (_, index) => {
    ...
    const items = calendarEventsFor(day);
    const marks = [...new Set(items.map((event) => event.type))].slice(0, 4)
      .map((type) => `<i class="calendar-week-mark ${type.toLowerCase().replaceAll(' ', '-')}"></i>`).join('');
    const isSelected = dateKey(day) === dateKey(selectedDay);
    const isToday = dateKey(day) === dateKey(today);
    return `<button class="calendar-week-day${isSelected ? ' active' : ''}" type="button" data-calendar-day="${dateKey(day)}" aria-pressed="${isSelected}"><span>${isToday ? 'Dzisiaj' : formatDate(day, { weekday: 'long' })}</span><b>${day.getDate()} ${formatDate(day, { month: 'short' }).replace('.', '')}</b><em>${items.length}</em><small>${marks}</small></button>`;
  }).join('');
}
...
function renderCalendar() { const list = calendarEventsFor(selectedDay).filter((event) => filter === 'Wszystkie' || event.type === filter); $('#selectedDate').textContent = formatDate(selectedDay); renderCalendarWeek(); const heading = '<div class="event-table-head"><span>Godzina</span><span>Wydarzenie</span><span>Typ</span><span>Lokalizacja</span><span>Status</span></div>'; $('#dailyEvents').innerHTML = heading + (list.length ? list.map(eventRow).join('') : `<div class="empty-state"><span>✦</span><p>Brak wydarzeń w tej kategorii.</p></div>`); refreshBossArtwork($('#dailyEvents')); }
function miniRow(event) { const sameDay = event.date === (event.isOlympiadSchedule ? olympiadLocalDate() : dateKey(today)); return `<div class="mini-event" data-boss-name="${safe(artworkName(event))}"><time>${sameDay ? event.time : formatDate(dateFromEvent(event), { day: '2-digit', month: 'short', ...(event.isOlympiadSchedule ? { timeZone: 'Europe/Warsaw' } : {}) })}</time><div><b>${safe(event.name)}</b><span>${event.type}${event.location ? ` · ${safe(event.location)}` : ''}</span>${event.isOlympiadSchedule ? `<span class="olympiad-window">${event.timeRange}</span>` : ''}</div></div>`; }
...
function renderNext() { const event = getUpcoming()[0]; const nextId = event?.id ?? null; const changed = nextId !== featuredEventId; featuredEventId = nextId; const announceChange = () => { if (changed && typeof window !== 'undefined') window.dispatchEvent(new CustomEvent('orzel:featured-event-updated', { detail: { id: nextId } })); }; applyBossArtwork($('.event-art-large'), artworkName(event)); if (!event) { $('#nextName').textContent = 'BRAK NADCHODZĄCYCH WYDARZEŃ'; $('#nextType').textContent = 'KALENDARZ KLANU'; $('#nextType').className = 'type-chip'; $('#nextMeta').textContent = 'Dodaj nowe wydarzenie w panelu administratora.'; $('#nextDescription').textContent = 'Gdy wydarzenie zostanie zaplanowane, pojawi się tutaj z pełnym odliczaniem.'; $('#nextStatus').textContent = 'OCZEKUJE'; $('#nextStatus').className = 'live-status zakończone'; announceChange(); return; } $('#nextName').textContent = event.name; $('#nextType').textContent = event.type; $('#nextType').className = `type-chip ${event.type.toLowerCase().replaceAll(' ', '-')}`; $('#nextMeta').textContent = eventWindowLabel(event); $('#nextDescription').textContent = event.description || ''; $('#nextStatus').textContent = eventStatus(event); $('#nextStatus').className = `live-status ${eventStatus(event).toLowerCase().replaceAll(' ', '-')}`; announceChange(); }
function updateCountdown() { const event = getUpcoming()[0]; $('#countdownLabel').textContent = event?.isOlympiadSchedule && eventStatus(event) === 'OLYMPIAD ACTIVE' ? 'Do zakończenia' : 'Do rozpoczęcia'; if (!event) { $('#countdown').innerHTML = '<b>00</b><i>:</i><b>00</b><i>:</i><b>00</b><i>:</i><b>00</b>'; return; } const currentStatus = eventStatus(event); const activeWindow = (currentStatus === 'RESPAWN WINDOW ACTIVE' || currentStatus === 'SIEGE ACTIVE') && event.endAt; const target = event.isOlympiadSchedule ? olympiadCountdownTarget(event) : activeWindow ? new Date(event.endAt) : dateFromEvent(event); let seconds = Math.max(0, Math.floor((target - Date.now()) / 1000)); const values = [Math.floor(seconds / 86400), Math.floor((seconds %= 86400) / 3600), Math.floor((seconds %= 3600) / 60), seconds % 60]; $('#countdown').innerHTML = values.map((value, index) => `<b>${pad(value)}</b>${index < 3 ? '<i>:</i>' : ''}`).join(''); $('#nextStatus').textContent = currentStatus; }
...
function openSiegeDialog(castle) {
  ...
  $('#siegeScheduleDialogHint').textContent = 'Podaj znany termin. Kolejne miesiące zostaną wyliczone automatycznie.';
...
async function initializeApp() {
  try {
    ...
    $('#calendarWeek')?.addEventListener('click', (event) => {
      ...
      if (!button) return;
      selectedDay = new Date(`${button.dataset.calendarDay}T00:00:00`);
      renderCalendar();
    });
