...
(function bootPublicCalendarSignups(){
  try {
    ...
    style.textContent = `
      ...
      .public-signup-count .psc-maybe,.public-signup-count .psc-no{display:inline-flex!important;align-items:center!important;min-height:22px!important;padding:5px 7px!important;border-radius:3px!important;font-size:9px!important;font-weight:900!important;text-transform:uppercase!important;letter-spacing:.04em!important}
      .public-signup-count .psc-maybe{border:1px solid #8a6a2d!important;background:#211a0d!important;color:#f0c86b!important}
      .public-signup-count .psc-no{border:1px solid #79342f!important;background:#21100e!important;color:#e77971!important}
    ...
    function render(){
      try {
        document.querySelectorAll('#dailyEvents .event-row').forEach(row => {
          ...
          const html = `<span class="psc-icon">♙</span><span class="psc-main"><b>${lists.yes.length}</b> zapisanych</span>${lists.maybe.length ? `<span class="psc-maybe">${lists.maybe.length} może</span>` : ''}${lists.no.length ? `<span class="psc-no">${lists.no.length} nie będzie</span>` : ''}<span class="psc-chevron">›</span>`;
