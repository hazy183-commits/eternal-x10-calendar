  ...
  return async request => {
    ...
    if (url.pathname === '/__preview-login' && request.method === 'POST') {
      ...
      try {
        ...
        const nickname = String(form.get('nickname') || '').slice(0,120).trim().toLowerCase().replace(/[^a-z0-9_-]/g,'');
        const password = String(form.get('password') || '');
        if (!nickname || !password || password.length > 256) return loginPage('Sprawdź nick i hasło.');
        const response = await fetcher(`${env.VITE_SUPABASE_URL}/auth/v1/token?grant_type=password`, { method: 'POST', headers: { apikey: env.VITE_SUPABASE_PUBLISHABLE_KEY, 'Content-Type': 'application/json' }, body: JSON.stringify({ email: `${nickname}@members.orzelbialy.local`, password }), signal: AbortSignal.timeout(8000) });
