-- Add the two permanent clan activities to the public calendar and Discord reminders.

begin;

CREATE OR REPLACE FUNCTION public.discord_notification_candidates(reference_at timestamp with time zone DEFAULT now())
 RETURNS TABLE(event_key text, event_id bigint, event_name text, event_type text, event_location text, start_at timestamp with time zone, reminder_enabled boolean, reminder_minutes integer)
 LANGUAGE sql
...
AS $function$
  ...
  manual_events as (
    select
      ...
      case when lower(btrim(coalesce(nullif(e.boss, ''), e.name))) in ('queen ant', 'core', 'orfen', 'baium', 'zaken', 'frintezza', 'antharas', 'valakas') then 'Epic RB' else e.type end as event_type,
  ...
  manual_bosses as (
    select
      ...
      case when lower(btrim(b.boss)) in ('queen ant', 'core', 'orfen', 'baium', 'zaken', 'frintezza', 'antharas', 'valakas') then 'Epic RB' else 'RB' end as event_type,
      case b.boss
  ...
  clan_activities as (
    select
      'clan-activity-pvp-' || to_char(d::date, 'YYYY-MM-DD') as event_key,
      null::bigint as event_id,
      'Klanowe PVP'::text as event_name,
      'Event'::text as event_type,
      'Strefa klanu'::text as event_location,
      (d::date + time '20:20') at time zone 'Europe/Warsaw' as start_at
    from generate_series(
      (reference_at at time zone 'Europe/Warsaw')::date,
      (reference_at at time zone 'Europe/Warsaw')::date + 14,
      interval '1 day'
    ) d
    ...
    select
      'clan-activity-colloseum-' || to_char(d::date, 'YYYY-MM-DD') as event_key,
      null::bigint as event_id,
      'Ćwiczenia Colloseum'::text as event_name,
      'Event'::text as event_type,
      'Strefa klanu'::text as event_location,
      (d::date + time '20:20') at time zone 'Europe/Warsaw' as start_at
    from generate_series(
  ...
  needed_rb as (
    select
      ...
      case when lower(btrim(r.boss_name)) in ('queen ant', 'core', 'orfen', 'baium', 'zaken', 'frintezza', 'antharas', 'valakas') then 'Epic RB' else 'RB' end as event_type,
  ...
  all_candidates as (
    ...
    union all select * from clan_activities
